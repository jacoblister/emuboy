...
class CPU {
    ...
    constructor(interrupt: interrupt.Interrupt, mapper: mapper.Mapper) {
        ...
        this.REG = <int[]>[]
        for (let i: int = 0; i < 8; i = i + 1) { this.REG.push(0) }
    ...
    executeOp(): int {
        ...
        if (op <= 0x3F) {
            if ((op & 0x0F) == 0x00) {
                ...
                if (op == 0x20 || op == 0x30) { // JP NZ/NC,r8
                    let flag: int = (op & 0x10) ? FLAG_C : FLAG_Z
                    let val: int = this.mapper.read(this.PC)
            ...
            if ((op & 0x0F) == 0x01) { // LD xx, d16
                let dst: int = (op & 0x70) >> 3
                let dl: int = this.mapper.read(this.PC)
                let dh: int = this.mapper.read(this.PC + 1)
            ...
            if ((op & 0x0F) == 0x02) { // LD xx, A
                ...
                if (op == 0x22 || op == 0x32) { // LD (HL+/-),A
                    ...
                    let inc: int = (op & 0x10) >> 4 ? 0xFFFF : 0x0001
                    let res: int = ((this.REG[_H] << 8 | this.REG[_L]) + inc) & 0xFFFF
                    this.REG[_H] = res >> 8
            ...
            if ((op & 0x07) == 0x03) { // INC/DEC xx
                let inc: int = op & 0x08 ? 0xFFFF : 0x0001
                if ((op & 0x30) == 0x30) {
                    this.SP = (this.SP + inc) & 0xFFFF
                    return 8
                }
                let dst: int = (op & 0x70) >> 3
                let data: int = (this.REG[dst] << 8 | this.REG[dst + 1])
            ...
            if ((op & 0x06) == 0x04) { // INC/DEC x
                let isDec: int = op & 0x01
                let dst: int = (op & 0x38) >> 3
                let inc: int = isDec ? 0xFF : 0x01
                let val: int = this.REG[dst]
                if (dst == _HL) {
                    val = this.mapper.read(this.REG[_H] << 8 | (this.REG[_L]))
                }
                let valPost: int = val + inc & 0xFF
                let half: int = (val >> 4) ^ (valPost >> 4) ? 1 : 0
            ...
            if ((op & 0x0F) == 0x08) {
                if (op == 0x08) { // LD (a16), SP
                    let addr: int = this.mapper.read(this.PC + 1) << 8 | this.mapper.read(this.PC)
                    this.PC = (this.PC + 2) & 0xFFFF
            ...
            if ((op & 0x0F) == 0x09) { // ADD HL, xx
                let src: int = (op & 0x70) >> 3
                let hl: int = this.REG[_H] << 8 | this.REG[_L]
                let val: int = op == 0x39 ? this.SP : this.REG[src] << 8 | this.REG[src + 1]
                let carry: int = (hl + val) >> 16
                let half: int = ((hl & 0xFFF) + (val & 0xFFF)) >> 12
                val = (val + hl) & 0xFFFF
                this.REG[_H] = val >> 8
            ...
            if ((op & 0x0F) == 0x0F) {
                if (op == 0x0F) { // RRCA
                    let carry: int = this.REG[_A] & 0x01
        ...
        if ((op >= 0x80 && op <= 0xBF) || (op >= 0xC0 && (op & 0x07) == 0x06)) {
            ...
            if (alOp == 0) { // ADD A,X
                let valPost: int = this.REG[_A] + val
                let half: int = ((this.REG[_A] & 0x0F) + (val & 0x0F)) >> 4
                let carry: int = valPost > 0xFF ? 1 : 0
                this.REG[_A] = valPost & 0xFF
                this.REG[_F] = (this.REG[_A] == 0 ? FLAG_Z : 0) | (half ? FLAG_H : 0) | (carry ? FLAG_C : 0)
            }
            if (alOp == 1) { // ADC A,X
                let valPost: int = this.REG[_A] + val + (this.REG[_F] & FLAG_C ? 1 : 0)
                let half: int = ((this.REG[_A] & 0x0F) + (val & 0x0F) + (this.REG[_F] & FLAG_C ? 1 : 0)) >> 4
                let carry: int = valPost > 0xFF ? 1 : 0
                this.REG[_A] = valPost & 0xFF
                this.REG[_F] = (this.REG[_A] == 0 ? FLAG_Z : 0) | (half ? FLAG_H : 0) | (carry ? FLAG_C : 0)
            }
            if (alOp == 2) { // SUB A,X
                let valPost: int = this.REG[_A] + ((val ^ 0xFF) + 1)
                let half: int = ((this.REG[_A] & 0x0F) - (val & 0x0F)) < 0 ? 1 : 0
                let carry: int = this.REG[_A] - val < 0 ? 1 : 0
                this.REG[_A] = valPost & 0xFF
                this.REG[_F] = (this.REG[_A] == 0 ? FLAG_Z : 0) | FLAG_N | (half ? FLAG_H : 0) | (carry ? FLAG_C : 0)
            }
            if (alOp == 3) { // SBC A,X
                let valPost: int = this.REG[_A] + ((val ^ 0xFF) + 1) + (this.REG[_F] & FLAG_C ? 0xFF : 0x00)
                let half: int = ((this.REG[_A] & 0x0F) - (val & 0x0F) - (this.REG[_F] & FLAG_C ? 1 : 0)) < 0 ? 1 : 0
                let carry: int = this.REG[_A] - val - (this.REG[_F] & FLAG_C ? 1 : 0) < 0 ? 1 : 0
                this.REG[_A] = valPost & 0xFF
                this.REG[_F] = (this.REG[_A] == 0 ? FLAG_Z : 0) | FLAG_N | (half ? FLAG_H : 0) | (carry ? FLAG_C : 0)
            }
            if (alOp == 4) { // AND A,X
        ...
        if (op >= 0xC0) {
            if ((op & 0x0F) == 0x00) {
                ...
                if (op == 0xE0) {
                    let addr: int = 0xFF00 | this.mapper.read(this.PC)
                    this.PC = (this.PC + 1) & 0xFFFF
            ...
            if ((op & 0x0F) == 0x08) {
                ...
                if (op == 0xE8) { // ADD SP, r8
                    ...
                    val = val > 127 ? 0xFFFF - (255 - val) : val
                    this.PC = (this.PC + 1) & 0xFFFF
                    let carry: int = ((this.SP & 0xFF) + (val & 0xFF)) >> 8
                    let half: int = ((this.SP & 0x0F) + (val & 0x0F)) >> 4
                ...
                if (op == 0xF8) { // LD HL, SP+r8
                    ...
                    let carry: int = ((this.SP & 0xFF) + (val & 0xFF)) >> 8
                    let half: int = ((this.SP & 0x0F) + (val & 0x0F)) >> 4
                    let addr: int = (this.SP + val) & 0xFFFF
